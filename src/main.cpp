...
HardwareSerial MKBSerial(1);

void sendCommand(const uint8_t *cmd, size_t len)
{
    while (MKBSerial.available()) {
        MKBSerial.read();
    }
...
void setup()
{
    ...
    sendCommand(fd_cmd, sizeof(fd_cmd));
}
